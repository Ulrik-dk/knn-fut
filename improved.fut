...
-- guarantees num_pad_elms <= num_leafs-1
...
let build_balanced_tree [n] (P: [n]i32) (leaf_size: i32) : ([]i32, []i32) =
  ...
  let (tree, P, _, _, _) = loop (tree, P, T_ofs, seg_cnt, seg_len) while seg_len != leaf_size do
    ...
    let tree = scatter tree (iota num_new_nodes |> map (+T_ofs)) new_tree_segment
